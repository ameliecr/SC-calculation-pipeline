...
general_name=CraddockSCorr2Level
orig_file=Craddock_scorr_2level_120parcel
n_parcels=120
threshold=500
dilution=3
...
fslmaths $orig_file -kernel box 12 -dilD $orig_file-dil
...
echo ""
echo "Linearly transform the MNI152 nonlinear template with the 4 mm to the one with the 1 mm resolution"
flirt \
    -in MNI152_T1_4mm_brain \
    -ref $FSLDIR/data/standard/MNI152_T1_1mm_brain \
    -omat 4mm-to-1mm.mat \
    -interp trilinear \
    -cost mutualinfo \
...
applywarp \
    -i $orig_file-reoriented \
    -r $FSLDIR/data/standard/MNI152_T1_1mm_brain \
    -o $orig_file-MNI-1mm \
    --interp=nn \
    --premat=4mm-to-1mm.mat
